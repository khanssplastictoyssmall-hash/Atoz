...
def init_db():
    with db() as c:
        c.execute("""CREATE TABLE IF NOT EXISTS settings (id INT PRIMARY KEY, data JSONB NOT NULL)""")
        ...
        if c.execute("SELECT COUNT(*) FROM settings").fetchone()[0] == 0:
            c.execute("INSERT INTO settings VALUES (1, %s)", (json.dumps(DEFAULT_SETTINGS),))
